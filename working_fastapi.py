#!/usr/bin/env python3
"""
Working FastAPI implementation for Jagriti Case Search API
...
# Simple FastAPI-like implementation using Starlette directly
...
async def root(request):
    return JSONResponse({"message": "Jagriti Case Search API", "version": "1.0.0"})
...
async def get_commissions(request):
    ...
    try:
        ...
        return JSONResponse({"commissions": commissions_data})
    except Exception as e:
        ...
        return JSONResponse({"commissions": commissions})
...
async def search_cases(request, search_field: str):
    ...
    try:
        ...
        if not state or not commission or not search_value:
            return JSONResponse({"error": "Missing required fields: state, commission, search_value"}, status_code=400)
        
        # Get state and commission IDs
        states_data = await scraper.get_states()
        state_id = next((s["id"] for s in states_data if s["name"].upper() == state.upper()), None)
        if not state_id:
            return JSONResponse({"error": f"State '{state}' not found"}, status_code=400)
        
        commissions_data = await scraper.get_commissions(state_id)
        commission_id = next((c["id"] for c in commissions_data if c["name"].upper() == commission.upper()), None)
        if not commission_id:
            return JSONResponse({"error": f"Commission '{commission}' not found for state '{state}'"}, status_code=400)
        ...
        try:
            ...
            return JSONResponse(cases_data)
        except Exception as e:
